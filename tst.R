                  ...
                  demean = TRUE, intercept = demean, series = NULL, ...) 
{
  ...
  for (m in order.min:order.max) {
    ...
    # aicccc <<- list( E, varE[[m - order.min + 1L]],  det(varE[[m - order.min + 1L]]), nser)
    xaic[m - order.min + 1L] <- n.used * log(det(varE[[m - 
                                                         order.min + 1L]])) + 2 * nser * (nser * m + intercept)
    # if(m == 8) {
    #   return(, A[[m - order.min + 1L]])
    # }
  }
  m <- if (aic) 
  ...
  # print(paste0("Raw intercept: ", xint))
  ...
  if (rescale) {
    xm <- xm * sc
    if (!is.null(xint)) {
      print(paste0("Raw intercept: ", xint, " sc: ", sc))
      print(paste0("Raw xm: ", xm/sc, " scaled xm: ", xm))
      xint <- xint * sc
    } 
    aa <- outer(sc, 1/sc)
    if (nser > 1L && m) 
