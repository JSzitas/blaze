...
#' @param demean Whether to also demean the series before fitting. See stats::ar() for details. Defaults to TRUE.
...
ar <- function(y, p = 1, include_mean = TRUE, include_drift = FALSE,
               demean = TRUE, xreg = list()) {
  ar_model <- new( blaze_ar, c(y), c(p), xreg, include_mean, include_drift, demean)
  ar_model$fit()
