...
// this just directly modifies coef
template <const bool seasonal,
          const bool transform> void arima_transform_parameters( Eigen::VectorXd &coef,
                                                                 const arima_kind &arma)
{
  ...
  if constexpr(transform) {
    ...
    v = mp + mq;
    if (msp > 0) {
      parameter_transform(coef, v, msp);
    }
  }
  if constexpr( seasonal ) {
    ...
    std::array<double, 200> phi;
    std::array<double, 200> theta;
