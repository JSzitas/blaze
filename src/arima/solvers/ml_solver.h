...
#include "arima/solvers/ml_kalman.h"
...
template <const SSinit ss_type, const bool has_xreg, const bool seasonal, const bool transform, typename scalar_t>
class ARIMA_ML_PROBLEM : public cppoptlib::function::Function<
                       scalar_t,
...
public:
  ...
  std::vector<scalar_t> get_residuals() const {
    return this->MLSolver.get_residuals();
  }
};

template <const SSinit ss_type, const bool has_xreg, const bool seasonal,
          const bool transform, typename scalar_t>
scalar_t solver_ml(std::vector<scalar_t> &y,
  ...
  residuals = ml_arima_problem.get_residuals();
...
template <const bool has_xreg, const bool seasonal,
          const bool transform, typename scalar_t>
scalar_t ml_solver(std::vector<scalar_t> &y,
                   structural_model<scalar_t> &model,
                   const bool intercept,
                   const bool drift,
                   std::vector<std::vector<scalar_t>> &xreg,
  ...
  if( ss_init == SSinit::Gardner ) {
    return solver_ml<SSinit::Gardner, has_xreg, seasonal, transform, scalar_t>(
        y, model, intercept, drift, xreg, kind, coef, kappa, residuals);
  }
