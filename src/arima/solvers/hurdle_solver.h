#ifndef ARIMA_HURDLE_SOLVER
#define ARIMA_HURDLE_SOLVER
...
template <typename scalar_t> sigmoid_link(
    const std::vector<scalar_t> & coef ) {
  scalar_t result = 0;
  for(auto & val:coef) {
    result += coef;
  }
...
template <typename C, typename scalar_t,
          const bool arima_only=false> scalar_t arima_css_ssq_binomial(
            ...
            for (size_t l = n_cond; l < n; l++) {
              ...
              tmp = y[l] < std::numeric_limits<scalar_t>() ? 0.0 : 1.0;
...
class ARIMA_LASSO_CSS_PROBLEM :
  public cppoptlib::function::Function<
    scalar_t,
    ARIMA_BINOMIAL_CSS_PROBLEM<has_xreg, seasonal, scalar_t, arima_only>> {
